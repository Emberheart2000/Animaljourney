	...
	GeneratePoints() {
		while (this.spawnPoints.length > 0) {
			...
			const spawnIndex = Math.floor(
				this.random() * this.spawnPoints.length
			);
			...
			for (let k = 0; k < this.maxCandidates; k++) {
				...
				const disp =
					Math.floor(this.random() * (this.radius + 1)) + this.radius;
				const candidate = spawnCentre.map(
					(val, i) => val + dir[i] * disp
				);
		...
		if (cX >= 0 && cX < this.width && cY >= 0 && cY < this.height) {
			...
			for (let x = searchStartX; x <= searchEndX; x++) {
				for (let y = searchStartY; y <= searchEndY; y++) {
					...
					if (pointIndex != 0) {
						const diff = candidate.map(
							(val, i) => val - this.points[pointIndex - 1][i]
						);
...
export const randomPosition = (radius = 100) => {
